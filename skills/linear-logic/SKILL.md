...
For code, use installed `cxc-dev` and the relevant verification owner. CXC owns authorized implementation, loops, and subagent routing; this audit does not start a loop or create a competing workflow. Return bounded findings to `linear-check` or `linear-run` when called by them.
...
Use the full assignment's authorization from [Integrations](../linear-plan/references/integrations.md#completion-follow-up-in-an-existing-execution-workflow). In an existing execution workflow, return evidence-backed corrections to the CXC/`linear-run` owner for implementation and recheck without another user permission round. Standalone audits return proposals. Route authorized planning corrections to `linear-plan`; a finding alone does not authorize new scope.
