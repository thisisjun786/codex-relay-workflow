...
the record intended. Every artifact path is absolute and normalised, which is the contract the relay's own manifest
enforces: a relative one is resolved against whichever directory a reader happens to be in, and an
