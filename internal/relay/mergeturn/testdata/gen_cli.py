"""Record the Python relay CLI's answers for the merge-turn commands (cli_cases.json).

  uv run --no-sync python internal/relay/mergeturn/testdata/gen_cli.py \
      > internal/relay/mergeturn/testdata/python_cli.json
...
cases = json.load(open("internal/relay/mergeturn/testdata/cli_cases.json"))
out = {}
for name, steps in cases.items():
    ...
    for step in steps:
        ...
        text = re.sub(r"tgt-[0-9a-f]{32}", replace, text)
