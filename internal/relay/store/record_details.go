...
// AttemptMessages lists frozen bytes in allocation order; a missing historical
// message stays absent instead of being rendered from current event state.
func (s *Store) AttemptMessages(ctx context.Context, eventID string) (_ []AttemptMessage, err error) {
	rows, err := s.q(ctx).QueryContext(ctx, `SELECT request_id,event_id,attempt_no,kind,message,rendered_at FROM attempt_messages WHERE event_id=? ORDER BY attempt_no`, eventID)
	if err != nil {
		return nil, fmt.Errorf("attempt messages %q: %w", eventID, err)
	}
	defer func() { err = errors.Join(err, rows.Close()) }()
	var messages []AttemptMessage
	for rows.Next() {
		var message AttemptMessage
		if scanErr := rows.Scan(&message.RequestID, &message.EventID, &message.Number, &message.Kind, &message.Message, &message.RenderedAt); scanErr != nil {
			return nil, fmt.Errorf("scan attempt message: %w", scanErr)
		}
		messages = append(messages, message)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("attempt messages rows: %w", err)
	}
	return messages, nil
}
...
func (s *Store) AttemptMessage(ctx context.Context, requestID string) (AttemptMessage, error) {
	...
	err := s.q(ctx).QueryRowContext(ctx, `SELECT request_id,event_id,attempt_no,kind,message,rendered_at FROM attempt_messages WHERE request_id=?`, requestID).Scan(&row.RequestID, &row.EventID, &row.Number, &row.Kind, &row.Message, &row.RenderedAt)
	if err != nil {
...
func (s *Store) Observation(ctx context.Context, threadID, turnID, status string) (Observation, error) {
	var row Observation
	err := s.q(ctx).QueryRowContext(ctx, `SELECT thread_id,turn_id,terminal_status,relationship_id,classification,event_id,observed_at FROM observations WHERE thread_id=? AND turn_id=? AND terminal_status=?`, threadID, turnID, status).Scan(&row.ThreadID, &row.TurnID, &row.TerminalStatus, &row.RelationshipID, &row.Classification, &row.EventID, &row.ObservedAt)
	if err != nil {
...
func (s *Store) RevisionLineage(ctx context.Context, relationshipID string, generation int64, eventID string) (RevisionLineage, error) {
	...
	err := s.q(ctx).QueryRowContext(ctx, `SELECT relationship_id,execution_generation,event_id,revision_hash,supersedes_hash,declared_by,recorded_at FROM revision_lineage WHERE relationship_id=? AND execution_generation=? AND event_id=?`, relationshipID, generation, eventID).Scan(&row.RelationshipID, &row.Generation, &row.EventID, &row.RevisionHash, &row.SupersedesHash, &row.DeclaredBy, &row.RecordedAt)
	if err != nil {
...
func (s *Store) AckEvidence(ctx context.Context, eventID string) (AckEvidence, error) {
	var row AckEvidence
	err := s.q(ctx).QueryRowContext(ctx, `SELECT event_id,tier,detail,attempts,last_reason,fingerprint,next_check_at,observed_at FROM ack_evidence WHERE event_id=?`, eventID).Scan(&row.EventID, &row.Tier, &row.Detail, &row.Attempts, &row.LastReason, &row.Fingerprint, &row.NextCheckAt, &row.ObservedAt)
	if err != nil {
