...
Before reassigning any remaining scope of an issue that already has a PR, align its boundary under [delivered issues](references/issue-boundaries.md#align-a-delivered-issue): compare the scope agreed at assignment, the criteria still open and the current owner, then either keep the remainder in that same unmerged PR or move it to a new issue with the merged one as its prerequisite. An instrumentation, partial or reference PR is not the delivery PR, and leaving the issue open with unchanged criteria is not alignment. Narrowing a delivered issue is carried by the authorization the request already gave; it is not a second approval step.
