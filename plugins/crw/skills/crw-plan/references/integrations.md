...
| Delivery evidence | The same-class delivery records a target was read from: their stable IDs and the times bounding each one, from assignment through the pull request and its review to the merge or acceptance. It is `no sample` where no comparable record existed at that reading, and it holds those records rather than any figure derived from them. Where a later change moved the target, these records stay as the reading they produced and the change source is what produced the date the item carries now; they are never rewritten to claim they set it, and `no sample` never stands for a date another source moved. |
