...
Each row fixes one document type's canonical location, the level whose record it is, the scope that location covers, what makes it change, and how the other side refers to it. Owner names an execution level rather than a class of document, because write authority comes from the assignment under [supervisor, parent and child scope](#supervisor-parent-and-child-scope); this table does not create a second ownership rule.
...
| Progress and status | Linear updates on the project or initiative | The level whose record it is | That subject's reporting period | Material progress, under the record rule already stated above | Not copied into the repository at all |
