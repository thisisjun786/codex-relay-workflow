...
| Disposition `ready_for_review`, but the registration names a generation the relay has moved off | `receipt_missing`, carrying `receiptEvidence` of `registration_generation_mismatch` | Yes. The hold is the same; the reason is not. `relationship.json` is create-once, so no receipt this session emits can satisfy the assignment, and the reason names the recovery - a new assignment for a fresh dispatch request id - instead of asking for a receipt |
