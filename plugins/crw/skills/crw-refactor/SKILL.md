...
An initiative supervisor coordinates the diagnostic handoff and user report; the project owner supplies technical judgment. Use Plan to deduplicate the selected candidate against current issues. Reuse an issue/task only when an undelivered issue already owns the candidate or the work is an in-scope review correction before delivery. A new change after a merged delivery needs a new issue and PR under [issue-to-PR mapping](../crw-plan/references/integrations.md#issue-to-pr-mapping); never reopen the completed delivery just to give the repair an owner. Within the authorized recording scope, create that issue and have its project parent dispatch it through Run; if recording or dispatch is outside the request, return the concrete handoff without writing or launching it. Implement in this task when the user explicitly chooses direct implementation, preserving the issue/PR mapping where it applies.

Carry the candidate's scope, evidence, contracts, baseline failures, same-cause search boundary and finish conditions together. A new task requires authorization under the host's task-creation rules. Follow the owning repository and CXC workflow without creating a second coordinator or changing another task's goal.
