...
Fix the rule source in the same pass. A rule is already agreed when Integrations states it or an accepted Linear decision carries it with an ID and a date you can cite. Anything else is a candidate rule: name it, route it to [crw-plan](../crw-plan/SKILL.md), and judge no record against it. An issue status, an assistant proposal and a newer local draft are none of them decisions, under [Linear holds canonical documents](../crw-plan/references/integrations.md#linear-holds-canonical-documents).
...
| Gap | The rule applies, the record falls short, and the supplement is clear under [Supplement within authority](#supplement-within-authority) |
