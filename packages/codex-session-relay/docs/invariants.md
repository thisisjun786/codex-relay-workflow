...
| I-176 | A returning identity is a second TENURE of the same relationship, not a collision and not a silent replay: it opens a new generation and records the lineage on both rows. A dead identity registered without a live predecessor is refused and told which route applies | `registry._returning_tenure`; the id is a pure function of parent, child and issue, so an A→B→A handback cannot mint a different one even in principle | implemented |
