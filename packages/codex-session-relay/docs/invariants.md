...
| I-169 | Any transition INTO a live status restores the lower level and passes the same ownership checks, so `paused` is not a quieter way back in than `active` | `linkage.apply_relationship_status_in` tests `lower in LIVE` rather than `lower == ACTIVE` on both the release and the restore side | implemented |
