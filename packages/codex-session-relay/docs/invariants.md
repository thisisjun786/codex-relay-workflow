...
| I-165 | A task that takes a scope back carries the endpoint it is running from now, not the one from its previous tenure | `apply_binding_plan` and `handover` write `host_id`, `cwd` and `cxc_session` on reactivation rather than only the status, and `linkage-handover` accepts all three so a command-line caller can state them | implemented |
