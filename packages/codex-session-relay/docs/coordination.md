...
- **Required checks are restated, not discovered.** `merge-turn-check --required` is the
  ...
  that `merge-evidence` read from the branch's effective rules, taken from the report
  `merge-turn-check` will itself compare the head against, and the notice names that report. They
  are a proposal the caller restates, not a discovery: the check stores whatever `--required` its
  caller passes. Where no such reading is recorded for the granted head, target and base, the
  notice leaves a placeholder and names the `merge-evidence` reading to take instead.
